            ...
            <div class="card">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3 d-sm-flex align-items-center justify-content-between">
                  ...
                  <a data-toggle="modal" data-target="#addModal" class="d-none d-sm-inline-block btn btn-sm btn-primary text-white shadow-sm" onclick="add(2)">اضافة</a>
                </div>
                <div class="card-body">
                  <div>
                    @foreach ($contact as $c)
                    <form class="form text-right" action="{{ route('dashboard.settings.destroy', ['s' => $c->id]) }}" method="POST">
                      @csrf
                      @method('delete')
                      <div class="mb-3">
                        <label>{{ $c->name }}</label>
